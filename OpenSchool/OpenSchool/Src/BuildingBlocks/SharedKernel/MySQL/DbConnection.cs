...
public class DbConnection : IDbConnection
{
        ...
        try
        {
            if (_currentState == ConnectionState.Fetching || _currentState == ConnectionState.Executing)
            {
                using (var newConnection = new MySqlConnection(CoreSettings.ConnectionStrings[_dbConfig]))
                {
                    ...
                    {
                        _numberOfConnection++;
                        return await newConnection.QuerySingleOrDefaultAsync<T>(sql, param,
            ...
            _currentState = ConnectionState.Fetching;
            return await _connection.QuerySingleOrDefaultAsync<T>(sql, param, CurrentTransaction,
                commandTimeout: commandTimeout, commandType: commandType);
        }
        finally
        {
    ...
    public async Task<IEnumerable<T>> ExecuteAndGetResultAsync<T>(
        string sql,
        object param,
        int? commandTimeout = null,
        CommandType? commandType = CommandType.Text,
    ...
    public async ValueTask<MySqlBulkCopyResult> WriteToServerAsync<T>(
        DataTable dataTable,
        IList<T> entites,
        bool autoCommit = false,
        CancellationToken cancellationToken = default)
